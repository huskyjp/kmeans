from kmeans import euclidean_distance_between_data,\
    assign_data_to_closest_centroid, update_assignment,\
    mean_of_points, update_centroids
import numpy as np
...
def test_eucliean_distance():
    ...
    assert euclidean_distance_between_data(data1, data2) == 2

    # random
    data1 = np.random.randn(100)
    data2 = np.random.randn(100)
    assert np.allclose(np.array(euclidean_distance_between_data(data1.tolist(),
                       data2.tolist())),
...
def test_assign_data():
    ...
    assert assign_data_to_closest_centroid(data_empty, centroids) \
        == "centroid1"
    assert assign_data_to_closest_centroid(data_random, centroids) \
        == "centroid2"
